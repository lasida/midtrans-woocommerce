(() => {
  ...
  // Example usage of registerPaymentMethod
  const id = "midtrans_sub_shopeepay";
  const name = window.wp.htmlEntities.decodeEntities(
    (0, window.wc.wcSettings.getSetting)("midtrans_sub_shopeepay", {}).title
  ) || window.wp.i18n.__("ShopeePay", "midtrans");

  const desc = (0, window.wc.wcSettings.getSetting)("midtrans_sub_shopeepay", {}).description || "";
  const img = (0, window.wc.wcSettings.getSetting)("midtrans_sub_shopeepay", {}).icon || "https://kepri.dompetdhuafa.org/wp-content/uploads/2025/02/download-1.png";

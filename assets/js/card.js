(() => {
  ...
  const id = "midtrans_sub_card";
  const name = window.wp.htmlEntities.decodeEntities(
    (0, window.wc.wcSettings.getSetting)("midtrans_sub_card", {}).title
  ) || window.wp.i18n.__("Credit/Debit Card", "midtrans");

  const desc = (0, window.wc.wcSettings.getSetting)("midtrans_sub_card", {}).description || "";
  const img = (0, window.wc.wcSettings.getSetting)("midtrans_sub_card", {}).icon || "";

(() => {
  ...
  const id = "midtrans_sub_echannel";
  const name = window.wp.htmlEntities.decodeEntities(
    (0, window.wc.wcSettings.getSetting)("midtrans_sub_echannel", {}).title
  ) || window.wp.i18n.__("Bank Transfer - Mandiri Bill Payment", "midtrans");

  const desc = (0, window.wc.wcSettings.getSetting)("midtrans_sub_echannel", {}).description || "";
  const img = (0, window.wc.wcSettings.getSetting)("midtrans_sub_echannel", {}).icon || "";

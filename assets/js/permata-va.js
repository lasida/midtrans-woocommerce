(() => {
  ...
  const id = "midtrans_sub_permata_va";
  const name = window.wp.htmlEntities.decodeEntities(
    (0, window.wc.wcSettings.getSetting)("midtrans_sub_permata_va", {}).title
  ) || window.wp.i18n.__("Bank Transfer - Permata VA", "midtrans");

  const desc = (0, window.wc.wcSettings.getSetting)("midtrans_sub_permata_va", {}).description || "";
  const img = (0, window.wc.wcSettings.getSetting)("midtrans_sub_permata_va", {}).icon || "";

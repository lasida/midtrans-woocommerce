(() => {
  ...
  const id = "midtrans_sub_bca_va";
  const name = window.wp.htmlEntities.decodeEntities(
    (0, window.wc.wcSettings.getSetting)("midtrans_sub_bca_va", {}).title
  ) || window.wp.i18n.__("Bank Transfer - BCA VA", "midtrans");

  const desc = (0, window.wc.wcSettings.getSetting)("midtrans_sub_bca_va", {}).description || "";
  const img = (0, window.wc.wcSettings.getSetting)("midtrans_sub_bca_va", {}).icon || "";

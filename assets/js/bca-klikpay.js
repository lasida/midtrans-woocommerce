(() => {
  ...
  const id = "midtrans_sub_bca_klikpay";
  const name = window.wp.htmlEntities.decodeEntities(
    (0, window.wc.wcSettings.getSetting)("midtrans_sub_bca_klikpay", {}).title
  ) || window.wp.i18n.__("BCA KlikPay", "midtrans");

  const desc = (0, window.wc.wcSettings.getSetting)("midtrans_sub_bca_klikpay", {}).description || "";
  const img = (0, window.wc.wcSettings.getSetting)("midtrans_sub_bca_klikpay", {}).icon || "";

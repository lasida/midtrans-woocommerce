(() => {
  ...
  // Usage specific for Other VA
  const id = "midtrans_sub_other_va";
  const name = window.wp.htmlEntities.decodeEntities(
    (0, window.wc.wcSettings.getSetting)("midtrans_sub_other_va", {}).title
  ) || window.wp.i18n.__("Bank Transfer - Other", "midtrans");

  const desc = (0, window.wc.wcSettings.getSetting)("midtrans_sub_other_va", {}).description || "";
  const img = (0, window.wc.wcSettings.getSetting)("midtrans_sub_other_va", {}).icon || "";

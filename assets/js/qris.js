(() => {
  ...
  // Usage specific for QRIS
  const id = "midtrans_sub_qris";
  const name = window.wp.htmlEntities.decodeEntities(
    (0, window.wc.wcSettings.getSetting)("midtrans_sub_qris", {}).title
  ) || window.wp.i18n.__("QRIS", "midtrans");

  const desc = (0, window.wc.wcSettings.getSetting)("midtrans_sub_qris", {}).description || "";
  const img = (0, window.wc.wcSettings.getSetting)("midtrans_sub_qris", {}).icon || "";

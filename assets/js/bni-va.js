(() => {
  ...
  const id = "midtrans_sub_bni_va";
  const name = window.wp.htmlEntities.decodeEntities(
    (0, window.wc.wcSettings.getSetting)("midtrans_sub_bni_va", {}).title
  ) || window.wp.i18n.__("Bank Transfer - BNI VA", "midtrans");

  const desc = (0, window.wc.wcSettings.getSetting)("midtrans_sub_bni_va", {}).description || "";
  const img = (0, window.wc.wcSettings.getSetting)("midtrans_sub_bni_va", {}).icon || "";
